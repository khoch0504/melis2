...
class ListController extends MelisAbstractActionController
{
    ...
    public function renderToolContentAction()
    {
        ...
        $melisTool->setMelisToolKey('cmtestservices', 'cmtestservices_tools');

        $columns = $melisTool->getColumns();
        $translator = $this->getServiceManager()->get('translator');
        $columns['actions'] = ['text' => $translator->translate('tr_cmtestservices_common_table_column_action')];

        $view->tableColumns = $columns;
        $view->toolTable = $melisTool->getDataTableConfiguration('#cmTestServicesTableContent', true, false, ['order' => [[ 0, 'desc' ]]]);
    ...
    public function getListAction()
    {
        ...
        if($this->getRequest()->isPost()){
            ...
            $tableData = $cmTestServicesService->getList($start, $length, $melisTool->getSearchableColumns(), $search, $selCol, $sortOrder, $langId)->toArray();
            $dataCount = $cmTestServicesService->getList(null, null, $melisTool->getSearchableColumns(), $search, null, 'ASC', $langId, true)->current();
    ...
    {
        $this->getEventManager()->trigger('cmtestservices_delete_end', $this, $this->getRequest());

        return new JsonModel([
            'success' => true,
            'textTitle' => 'tr_cmtestservices_delete_item',
            'textMessage' => 'tr_cmtestservices_delete_success',
        ]);
    }
}