...
class ListController extends MelisAbstractActionController
{
    ...
    public function renderToolContentAction()
    {
        ...
        $melisTool->setMelisToolKey('cmservices', 'cmservices_tools');

        $columns = $melisTool->getColumns();
        $translator = $this->getServiceManager()->get('translator');
        $columns['actions'] = ['text' => $translator->translate('tr_cmservices_common_table_column_action')];

        $view->tableColumns = $columns;
        $view->toolTable = $melisTool->getDataTableConfiguration('#cmServicesTableContent', true, false, ['order' => [[ 0, 'desc' ]]]);
    ...
    public function getListAction()
    {
        ...
        if($this->getRequest()->isPost()){
            ...
            $tableData = $cmServicesService->getList($start, $length, $melisTool->getSearchableColumns(), $search, $selCol, $sortOrder, $langId)->toArray();
            $dataCount = $cmServicesService->getList(null, null, $melisTool->getSearchableColumns(), $search, null, 'ASC', $langId, true)->current();
    ...
    {
        $this->getEventManager()->trigger('cmservices_delete_end', $this, $this->getRequest());

        return new JsonModel([
            'success' => true,
            'textTitle' => 'tr_cmservices_delete_item',
            'textMessage' => 'tr_cmservices_delete_success',
        ]);
    }
}