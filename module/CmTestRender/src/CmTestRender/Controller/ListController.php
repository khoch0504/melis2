...
class ListController extends MelisAbstractActionController
{
    ...
    public function renderToolContentAction()
    {
        ...
        $melisTool->setMelisToolKey('cmtestrender', 'cmtestrender_tools');

        $columns = $melisTool->getColumns();
        $translator = $this->getServiceManager()->get('translator');
        $columns['actions'] = ['text' => $translator->translate('tr_cmtestrender_common_table_column_action')];

        $view->tableColumns = $columns;
        $view->toolTable = $melisTool->getDataTableConfiguration('#cmTestRenderTableContent', true, false, ['order' => [[ 0, 'desc' ]]]);
    ...
    public function getListAction()
    {
        ...
        if($this->getRequest()->isPost()){
            ...
            $tableData = $cmTestRenderService->getList($start, $length, $melisTool->getSearchableColumns(), $search, $selCol, $sortOrder, $langId)->toArray();
            $dataCount = $cmTestRenderService->getList(null, null, $melisTool->getSearchableColumns(), $search, null, 'ASC', $langId, true)->current();
        }
    ...
    {
        $this->getEventManager()->trigger('cmtestrender_delete_end', $this, $this->getRequest());

        return new JsonModel([
            'success' => true,
            'textTitle' => 'tr_cmtestrender_delete_item',
            'textMessage' => 'tr_cmtestrender_delete_success',
        ]);
    }
}